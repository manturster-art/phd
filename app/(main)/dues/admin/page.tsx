// SCR-060 + SCR-064 — 회비 매트릭스 + 입금 신고 컨펌 (임원).
...
export default async function DuesAdminPage({
  ...
  const matrix = activeTermId
    ? await listDuesMatrix(supabase, activeTermId).catch(() => [])
    : [];
  const pending = await listPendingDuesPayments(supabase).catch(() => []);
  ...
  return (
    <>
      ...
      <div className="space-y-5 py-4">
        <PendingApprovalsSection pending={pending} />
